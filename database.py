...
<<<<<<< HEAD
    ...
    def ensure_connection(self) -> None:
        """Ensure psycopg2 connection exists and is open; reconnect if needed."""
        try:
...
<<<<<<< HEAD
    ...
    async def get_all_users(self) -> List[Dict]:
        ...
        def _get():
            ...
            for row in rows:
                ...
                display_name = ' '.join(parts) if parts else (username or f"User {row['user_id']}")
                users.append({'id': row['user_id'], 'name': display_name})
            return users
    ...
    async def get_system_stats(self) -> Dict:
        ...
        def _get():
            ...
            cursor.execute('SELECT COUNT(*) FROM business_snapshots WHERE DATE(created_at) = CURRENT_DATE')
    ...
    async def get_advice(self) -> List[str]:
        """Получение советов из базы данных"""
        def _get():
            cursor = self.conn.cursor()
            cursor.execute('SELECT advice1, advice2, advice3, advice4 FROM business_snapshots WHERE advice1 IS NOT NULL AND advice1 != \'\' LIMIT 1')
            row = cursor.fetchone()
            
            if row and any(row.values()):
                advice_list = [advice for advice in row.values() if advice and advice.strip()]
                # Если советов меньше 4, дополняем дефолтными
                default_advice = [
                ...
                while len(advice_list) < 4:
                    advice_list.append(default_advice[len(advice_list)])
                
                return advice_list[:4]
            else:
                # Возвращаем дефолтные советы если в БД нет данных
                return [
                    "Проанализируйте категории расходов в разделе аналитики",
                    "Отслеживайте динамику привлечения клиентов", 
                    "Получайте персональные советы для вашего бизнеса",
