...
2. Если все "ТРЕБУЕМЫЕ ДАННЫЕ" есть, то ищи недостающие данные из "ДОПОЛНИТЕЛЬНЫХ ДАННЫХ".
3. Сформулируй 1-2 максимально конкретных и коротких вопроса по ОДНОЙ, НАИБОЛЕЕ ВАЖНОЙ ОТСУТСТВУЮЩЕЙ метрике, чтобы получить ее значение.
4. Если ВСЕ данные из обоих списков собраны, верни ТОЛЬКО ОДНО СЛОВО: ENOUGH_DATA.
